#!/usr/bin/env python3
"""
Load foundational dataset with separate base pricing structure - FIXED VERSION
foundational_dataset_v1.csv → store_base_prices table (매장별 기본가격)
"""

import csv
from sqlalchemy import MetaData, Table, insert, delete, Column, Integer, String, Decimal, DateTime, UniqueConstraint
...
def load_foundational_data_v2():
    ...
    with Session() as session:
        try:
            ...
            # Clear existing foundational data (in correct order)
            ...
            with open(csv_path, 'r', encoding='utf-8') as file:
                ...
                for row_num, row in enumerate(reader, 1):
                    ...
                    try:
                        ...
                        # Collect unique products (use first occurrence for basic info)
                        if product_id not in products_data:
                            ...
                            products_data[product_id] = {
                                ...
                                'basePrice': base_price,  # 첫 번째 매장 가격을 기본으로
                        ...
                        # Collect base prices per store per product (THIS IS THE KEY!)
            ...
            # Insert foundational data in correct order
            if categories_data:
            ...
            print("\n📋 Data Structure:")
            print("   - products: Basic product info (single basePrice per product)")
            print("   - store_base_prices: Store-specific pricing (400 entries)")
            print("   - storeOfferings: Reserved for ETL discount data")
            
            # Show sample data
            print(f"\n🔍 Sample base price data:")
            sample_product = list(products_data.keys())[0] if products_data else None
            if sample_product:
                sample_prices = [bp for bp in base_prices_data if bp['productId'] == sample_product]
                for sp in sample_prices[:3]:
                    store_name = [s['storeName'] for s in stores_data.values() if s['storeId'] == sp['storeId']][0]
                    print(f"   {sample_product}: {store_name} = ${sp['basePrice']}")
...
if __name__ == "__main__":
    ...
    if success:
        print("\n✅ Success! Database structure:")
        print("   - Foundational data: products + store_base_prices")  
        print("   - ETL discount data: storeOfferings (empty, ready for ETL)")
        print("   - API will show store-specific base prices")
        print("   - When ETL runs, discounts will be applied on top of base prices")
    else:
