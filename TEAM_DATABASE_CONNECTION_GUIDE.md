...
## How to Connect to Mimi's Local MySQL Database

This guide explains how team members can connect to the shared local MySQL database running on Mimi's computer.

---

## 📋 Prerequisites

1. **Network Connection**: You must be on the same local network as Mimi's computer
...
1. **Check Network**: Make sure you're on the same WiFi/network as Mimi
2. **Check Mimi's Computer**: Ensure Mimi's computer is on and the Docker containers are running
3. **Firewall**: Mimi might need to allow port 3306 through firewall
...
- Contact Mimi to verify the database is running

### 🔍 Check Mimi's Database Status
Mimi can run this to check if the database is accessible:
```bash
...
- **Port Access**: Port 3306 must be accessible from Mimi's computer
- **IP Address**: If `172.16.38.147` doesn't work, ask Mimi for the current IP
...
2. **Backup Important Data**: Ask Mimi to backup before major changes
...
2. Verify you're on the same network as Mimi
3. Contact Mimi to verify database status
